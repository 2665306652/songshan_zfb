...
<template>
	<view class="ynGalleryPanel" >
		<view class="galleryBg" :style="{'background-image': 'url(' + images[activeviewindex].url + ')'}"></view>
			...
			<scroll-view class="scroll-view" scroll-x="true" scroll-with-animation="true" :scroll-into-view="toview" :scroll-left="scrollLeft" @scroll="scroll">
				...
				<view class="scroll-view-panle" @touchend='touchEndHandle' @touchstart='touchStartHandle' @touchmove='touchMoveHandle'>
					...
					<view :id="'imgviewpanel'+index" :ref="'imgviewpanel'+index" :class="[activeviewindex == index? 'trans-fadeout':  '']"
						...
						<!-- 图片框 居中属性-->
						<view style="display:flex; align-content:flex-end;justify-content: center; border-radius: 15px;position: relative;"
							  ...
							  <!-- 描述视图 -->
							  <view class="dec-wrap">
							  	<text class="dec-title" v-if="showdec">{{item.dec}}</text>
								<view class="icons-wrap">
									<text class="icon-wrap">
										<image src="../../../static/assents/tikect/img-icon.png" class="icon"></image>
										{{sviewlst.imgsview.length}}
									</text>
									<!-- <text class="icon-wrap">
										<image src="../../../static/assents/tikect/video-icon.png" class="icon"></image>
										3
									</text> -->
						...
						<!-- <view style="display: flex;justify-content: center;margin-bottom: 4px;border:  #008000 0px solid;min-height: 10px;">
							<text class="dec-title" v-if="showdec" style="align-self: center;font-size: 12px;color:white">{{item.dec}}</text>
						</view> -->
...
<script>
	...
	export default {
		...
		        let posLeft = this.sviewlst.imgsview[this.sviewlst.imgsview.length-1].l;
		        let posRight = this.sviewlst.imgsview[this.sviewlst.imgsview.length-1].r;
		        let posRes = (posRight-posLeft)/2 + posLeft;

		        // console.log('中锚点位置:'+cenpos); 
		        if (cenpos < posRes && this.screencenterpos < cenpos) {
